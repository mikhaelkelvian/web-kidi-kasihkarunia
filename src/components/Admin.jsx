...
const Admin = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-auto lg:h-screen ">
      ...
      <div
        id="sidebar"
        className="w-full lg:w-1/3 bg-blue-300 border-b lg:border-r lg:border-b-0 border-blue-500 shadow-lg"
      >
        <div className="flex flex-col mx-5 lg:mx-10 gap-2 py-5 lg:my-40">
          <h2 className="text-2xl lg:text-3xl font-semibold">
            Apa itu dashboard admin?
          </h2>
          <span className="h-[1px] bg-slate-500 opacity-75"></span>
          <p className="font-medium text-sm lg:text-base">
            Dashboard admin adalah tempat dimana kamu dapat menambahkan data
            produk mu secara otomatis dan tentunya cepat. Karena disini kamu
            hanya perlu memasukkan data-data uang diperlukan untuk menambahkan
            produk baru.
          </p>
          <div className="flex flex-col lg:flex-row gap-3 my-7">
            <a
              href="/"
              className="flex items-center justify-center gap-1 px-4 py-3 bg-blue-500 hover:bg-blue-600 transition duration-200 text-white rounded-lg"
            >
              <IoChevronBack size={25} color="#fff" /> Ke Beranda
            </a>
            <button
              onClick={handleLogout}
              className="flex items-center justify-center gap-3 px-4 py-3 bg-red-500 hover:bg-red-600 transition duration-200 text-white rounded-lg"
            >
      ...
      <div className="px-5 lg:px-14 flex flex-col w-full lg:w-2/3 overflow-auto">
        <h1 className="text-center text-2xl lg:text-4xl font-semibold my-8 lg:my-16">
          Dashboard Admin
        </h1>
        ...
        >
          <h1 className="text-xl lg:text-2xl font-medium">
            {isEditing ? "Edit Produk" : "Tambahkan Produk Baru Anda"}
          </h1>
          <input
            type="file"
            onChange={(e) => setImageFile(e.target.files[0])}
            className="w-full h-[150px] p-2 border-2 rounded border-slate-400"
            required
          />
          ...
          <button
            type="submit"
            className="w-full lg:w-3/4 mx-auto py-2 bg-blue-500 hover:bg-blue-600 text-white font-medium rounded-lg"
          >
          ...
          {isEditing && (
            <button
              ...
              className="w-full lg:w-3/4 mx-auto py-2 bg-none hover:bg-red-500 border-2 border-red-500 rounded-lg font-medium text-red-500 hover:text-white transition duration-150"
            >
              Batal
            </button>
          )}
        ...
        <div className="my-12">
          ...
          <ul className="flex flex-col space-y-4">
            {products.map((product) => (
              <div
                key={product.id}
                className="flex flex-col lg:flex-row justify-between p-4 bg-blue-200 border-2 border-blue-500 rounded-lg"
              >
                <div className="flex flex-col gap-1 w-full lg:w-3/4">
                  ...
                  {product.imageUrl && (
                    <img
                      ...
                      alt={product.name}
                      className="mt-2 h-32 lg:h-48 object-cover rounded"
                    />
                  )}
                </div>
